// Two-layer architecture unit test (node --test): judgment-layer semantic category (fixes read_file leak) +
// deduction-layer grey-zone fallback.
// Freezes multiple bypass variants of the previous round's real bug (read_file reading .env leaked) and verifies the
// deduction layer's grey zone doesn't misfire.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { WeiwenLawEngine } from '../src/core/engine.mjs';

// ---------------- Judgment layer: semantic category fixes read_file leak (previous round real-bug variants frozen) ----------------
...
// ---------------- Deduction layer: grey-zone causal-consequence fallback (where judgment layer can't give a certain verdict) ----------------
...
// ---------------- Deduction layer H fork — parallel — contrast (manuscript core structure special) ----------------
...
// ---------------- Deduction layer doesn't misfire on normal ops ----------------
