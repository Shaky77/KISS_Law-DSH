// Weiwen's Law engine unit test (node --test)
// Verifies all R/D/S/H/M adjudication paths and S monotonicity/barrel; deterministic, zero-dependency, no API key.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { WeiwenLawEngine } from '../src/core/engine.mjs';

// ---------------- S steady-state reserve: dual nature (time-mark irreversible + current value can rise/fall) ----------------
...
// ---------------- Integration: S accumulates after allow ----------------
...
// ---------------- S time-cycle model: same-topic events aggregate (prevent context overload, author 2026-08-19) ----------------
...
// ---------------- R anchor boundary regression ----------------
// R is objective rule (cosmic law / scientifically verified / social consensus); its boundary should be clear.
// Engineering must not distort the rule with fuzzy substring matching.
// Only recognize "standalone extension forms": .key followed by alphanumerics is NOT a credential (.keyfile/.keyboard don't hit);
// the grey zone (R doesn't hit) is handed to the deduction layer for risk adjudication, decision right returned to the user.
