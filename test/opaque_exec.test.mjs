// Auditability-direction criterion freeze test
// Proposition: an action whose execution content is unreadable before execution, regardless of what it decodes to, has a
// causal direction of "S cannot be guaranteed". Therefore the deduction layer must: ① S+1 not established (cannot prove
// steady-state increment) ② D path deterministically erodes → deny.
// The criterion identifies the objective pipeline structure "opaque source → transform/evaluate → execution sink", not the
// restored content.
...
// ---------------- I. Unauditable execution: direction judged bad, must deny (deduction layer) ----------------
const OPAQUE_CASES = [
...
// ---------------- II. Misfire boundary: looks similar but direction is auditable, must not block ----------------
...
// ---------------- III. Direction vs content: disguised as "harmless content" still blocked ----------------
...
// ---------------- IV. Sink existence (third segment of the three-segment structure): writing a doc ≠ writing an exec channel ----------------
// Self-found misfire (2026-08-24): judging only by text form would also judge "writing a doc containing a curl|bash
// install snippet" as execution. Whether text gets executed depends on which sink it lands in —— this is still a direction
// criterion (structure), not a content criterion.
...
// ---------------- IV. Risk < Weiwen's Law < steady-state (no =, less/equal counts as fail) ----------------
// allow's only condition: S+1 strictly established AND D erosion strictly 0 (both hold). The S-branch prediction must
// participate in the decision, no longer discarded.
// Auditability extension: not just form (isOpaqueExec); unknown variable references / this-session disk-write references
// are equally unauditable.
...
// ---------------- V. TRIAL2 regression freeze (2026-08-25 · scenario swap exposed and fixed) ----------------
