// receipt-gate.test.mjs — 回执门（tools/post-execute）的结构回归锁（2026-09-21）
// ---------------------------------------------------------------------------
...
test('breakAtReceipt：偏离未达阈值 ⇒ 返回空（门不得拦）', () => {
  ...
  assert.equal(e.breakAtReceipt(), null, 'cap-2 及以下不得触发（阈值 5，当前 3 ⇒ 投影 4 < 5）');
});
...
test('breakAtReceipt：破窗在效 ⇒ 读数标记 broken（判据与 checkBreakWindow 同源：两条路径都认）', () => {
  // 路径①：显式止损态（R 层投影路线，2026-09-18）
  const a = new WeiwenLawEngine({ rigidAnchors: DEFAULT_RIGID_ANCHORS });
  a.windowBroken = true;
  assert.equal(a.breakAtReceipt()?.broken, true);

  // 路径②：累积已达阈值而 flag 未置（入账路线的常规形态）
  //   原实现只读 flag ⇒ 此路径被报成「即将达阈值（6/5）」：止损其实早已生效，却描述成尚未发生。
  const b = new WeiwenLawEngine({ rigidAnchors: DEFAULT_RIGID_ANCHORS });
  for (let i = 0; i < 5; i += 1) b.onFailure();
  const bw = b.breakAtReceipt();
  assert.ok(bw, '已达阈值必须有读数');
  assert.equal(bw.broken, true, '窗口已在效，不得再报"即将达阈值"');
  assert.equal(bw.streak, 6, '投影值 = 已累积 5 + 本次 1（读数如实，不美化）');
});

test('breakAtReceipt 是纯读：不写 failureStreak、不置 windowBroken（判据单点在引擎）', () => {
  const e = new WeiwenLawEngine({ rigidAnchors: DEFAULT_RIGID_ANCHORS });
  e.windowBroken = true;
  e.breakAtReceipt();
  e.breakAtReceipt();
  assert.equal(e.failureStreak, 0, '投影不得入账（入账仍归 onFailure / tools/result）');
  e.windowBroken = false;
  for (let i = 0; i < 4; i += 1) e.onFailure();
  e.breakAtReceipt();
  assert.equal(e.failureStreak, 4, '读数不得改变累积');
});
...
test('成功回执原样透传：不替换 content、不施加裁决（fail-open 边界）', async () => {
  ...
  assert.equal(out, NEXT, '成功结果不得被改写（哪怕偏离累积已很高）');
  assert.equal(await streakOf(tools), before, '门是纯读：不动状态');
  const rep = await toolNamed(tools, 'query_anchor_channel').execute({}, {});
  assert.equal(rep.adapter.receiptGate.seen, 1, '门被派发（覆盖证据）');
  assert.equal(rep.adapter.receiptGate.failedSeen, 0);
  assert.equal(rep.adapter.receiptGate.blocked, 0);
});
...
test('失败但未达阈值：放行原回执（不把每次普通报错都变成纠错回执）', async () => {
  ...
  const rep = await toolNamed(tools, 'query_anchor_channel').execute({}, {});
  assert.equal(rep.adapter.receiptGate.failedSeen, 1);
  assert.equal(rep.adapter.receiptGate.blocked, 0, '未达阈值不得阻断（否则普通报错被误当断点）');
});
...
test('阈值将达：回执被断成纠错错误，且原文与 BUG 身份一并带过（证据不得被毁）', async () => {
  ...
  assert.equal(out.closedLoop, true, 'M 闭环语义：重入前须 反推→溯源→修复');
  ...
  const rep = await toolNamed(tools, 'query_anchor_channel').execute({}, {});
  assert.equal(rep.adapter.receiptGate.blocked, 1);
  assert.equal(rep.adapter.receiptGate.lastStreak, '5/5');
});
...
test('fail-open：门内任何异常 ⇒ accept，绝不把健康调用弄成 error', async () => {
  const { ctx, handlers, tools } = mockCtx();
  apply(ctx);
  accrueFailures(handlers, 4);   // 让门走到会读取 content 的分支
  const hostile = {
    isError: true,
...
test('契约稳健：宿主未传 next ⇒ 也返回合法裁决（不得返回 undefined）', async () => {
  ...
  assert.equal(out.kind, 'accept', 'waterfall 监听器不得返回 undefined（宿主会去读 decision.additionalContexts）');
});

test('覆盖：成功与失败回执都经门（不是只给失败开的旁路）', async () => {
  const { ctx, handlers, tools } = mockCtx();
  apply(ctx);
  await handlers['tools/post-execute'](exec('a', {}), okReceipt(), next);
  await handlers['tools/post-execute'](exec('b', {}), badReceipt(), next);
  await handlers['tools/post-execute'](exec('c', {}), badReceipt('x'), next);
  const rep = await toolNamed(tools, 'query_anchor_channel').execute({}, {});
  assert.equal(rep.adapter.receiptGate.seen, 3);
  assert.equal(rep.adapter.receiptGate.failedSeen, 2);
});
