// Inner-H parking protocol unit test (author-fixed · 2026-08-30)
// Four protocol clauses:
//   ① Parallel dual-track: inner-H is an unknown x, parked in parallel without participating in outer-H deduction
//   ② Purpose not disclosed: the deduction purpose belongs to AI inner-H runtime logic, self-determined by AI, not
//     reported to the user (motive never exists from the start)
//   ③ Interception right: outer-H judged as deny → intercept directly, no need to wait for inner-H clarification, leave evidence slot
//   ④ Causal attribution: outer-H deduction result delivered together with inner-H parked state; thereafter causality
//     belongs to the user, AI stops deliberating
// Hard constraint: the review tier must NOT escalate to deny on "inner-H suspicious" — inner-H is parked, cannot be
// any basis for interception.
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { WeiwenLawEngine } from '../src/core/engine.mjs';

// ---------------- Protocol ①: inner-H does not participate in deduction ----------------
...
// ---------------- Protocol ②: purpose not disclosed ----------------
...
// ---------------- Protocol ③: interception right + leave evidence slot ----------------
...
// ---------------- Protocol ④: allow only reports status ----------------
...
// ---------------- Claim: reversal right belongs to the user ----------------
...
// ---------------- H-layer sovereignty boundary (e9e6b96 ported regression) ----------------
...
// ---------------- English-structure counterparts (2026-08-30 gap fix · align with English edition) ----------------
// e9e6b96's CFG_VERB/CFG_OBJECT, IMPERATIVE_ZH, SYS_SELF_REF are all Chinese;
// the three English injections below were passed through directly before the fix (verified by test).
