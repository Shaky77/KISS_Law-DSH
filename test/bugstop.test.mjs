...
// Verifies: BUG → halt → backtrack → trace → repair(verify) → reenter, forming a closed loop;
//   reentry is forbidden before repair, fundamentally stopping "backtrack-only-without-repair → infinite recursion".
...
// ---------------- Block infinite recursion: re-running with the original BUG is rejected; stubborn no-repair barrage hits 9 → human ----------------
...
// ---------------- After loop closes: reentry allowed ----------------
test('闭环：完成 反推→溯源→修复(验证) 后，重入放行', () => {
  ...
  // 修复：去除悖论标志。修复后的 call 须为**真实语义调用**（带真名真参数）——若用 {name:'reason',args:{}} 这类空调用属失真输入：路径1 归因必然判定「名中性 + 无可观测行为」，按铁律落 review，测不出「闭环闸门放行」这一意图。（2026-09-04 树视角复盘：闭环 verify 只验 M 枝⑥，不覆盖 R 枝，故重入时 R 枝照审。）
...
// ---------------- Core reinforcement: backtrack-but-no-repair still blocks reentry ----------------
